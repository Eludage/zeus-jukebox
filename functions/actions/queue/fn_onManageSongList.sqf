/*
 * Author: Eludage
 * Shows the Manage Song Lists overlay on top of the open Jukebox dialog.
 *
 * Arguments:
 * None
 *
 ...
 *
 * Example:
 * [] call ZeusJukebox_fnc_onManageSongList;
 */
...
uiNamespace setVariable ["ZeusJukebox_manageSongListsOverlayOpen", true];
...
// Populate with the latest playlist data every time the overlay opens
...
// while the overlay is open (re-enabled in onManageSongListClose.sqf). Mirrors the
// blocking mechanism used by onMusicListSettings.sqf / onTrackHistoryOpen.sqf.
private _disableIdcs = [
    // Preview
    15304, 15306, 15307, 15308, 15309, 15311, 15312,
    // Options
    15402, 15403, 15404, 15405, 15406,
    // Music List (including the gear button and the History button, so neither
    // can be re-clicked while this overlay is already showing)
    15502, 15503, 15505, 15506, 15507, 15508, 15509, 15510, 15511, 15512,
    // Currently Playing
    15605, 15606, 15607, 15608, 15609, 15610, 15611, 15612,
    // Queue (including the Manage Song List button itself, idc 15710)
...
// Move focus off the Manage Song List button and onto the overlay's close button
// so it doesn't stay highlighted/re-triggerable while the overlay is open
private _closeButton = _display displayCtrl 16003;
